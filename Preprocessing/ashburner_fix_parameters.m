function matlabbatch = ashburner_fix_parameters(runs, subjectid, wildcards, directories)
...
for crun = 1:length(runs) % for each run entered into this function..
    ...
    matlabbatch{6}.spm.spatial.normalise.write.subj.resample(crun) = cfg_dep(['Slice Timing: Slice Timing Corr. Images (Sess ' num2str(crun) ')'], substruct('.','val', '{}',{2}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('()',{crun}, '.','files'));
...
matlabbatch{3}.spm.util.imcalc.input(1)       = cfg_dep('Realign: Estimate & Reslice: Mean Image', substruct('.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','rmean'));
matlabbatch{3}.spm.util.imcalc.output         = 'ashburner_fixed_mean_functional';
matlabbatch{3}.spm.util.imcalc.outdir         = {''};
matlabbatch{3}.spm.util.imcalc.expression     = 'i1 + randn(size(i1))*50';
...
matlabbatch{4}.spm.spatial.coreg.estimate.source = {spm_select('ExtFPListRec', anat_directory, wildcards.anat)}; % path to anatomical image
matlabbatch{4}.spm.spatial.coreg.estimate.ref(1) = cfg_dep('Image Calculator: ImCalc Computed Image: output', substruct('.','val', '{}',{3}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','files'));
...
matlabbatch{5}.spm.spatial.preproc.channel.vols(1)  = cfg_dep('Coregister: Estimate: Coregistered Images', substruct('.','val', '{}',{3}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','cfiles'));
...
matlabbatch{5}.spm.spatial.preproc.tissue(1).tpm    = {[spm_segment_image ',1']};
...
matlabbatch{5}.spm.spatial.preproc.tissue(2).tpm    = {[spm_segment_image ',2']};
...
matlabbatch{5}.spm.spatial.preproc.tissue(3).tpm    = {[spm_segment_image ',3']};
...
matlabbatch{5}.spm.spatial.preproc.tissue(4).tpm    = {[spm_segment_image ',4']};
...
matlabbatch{5}.spm.spatial.preproc.tissue(5).tpm    = {[spm_segment_image ',5']};
...
matlabbatch{5}.spm.spatial.preproc.tissue(6).tpm    = {[spm_segment_image ',6']};
